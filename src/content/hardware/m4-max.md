...
At 64 GB M4 Max, that model loads with buffer for OS and inference overhead. At 36 GB base config, it technically loads but runs with tight headroom — the 64 GB configuration is worth the upgrade if 70B is your target.
...
At 36 GB: one 32B model loaded, or one 22B at Q8. Still a step above the Pro tier.
